...
class DebtTracker:
    ...
    def get_debts(self):
        cursor = self.conn.cursor()
        cursor.execute("SELECT id, name, amount, description, date, paid FROM debts")
    ...
    def add_debt(self, name, amount, description, date=None):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "INSERT INTO debts (name, amount, description, date, paid) VALUES (?, ?, ?, ?, ?)",
            (name, amount, description, date, 0)
        )
        self.conn.commit()
    
    def mark_debt_paid(self, debt_id):
        cursor = self.conn.cursor()
        cursor.execute("UPDATE debts SET paid=1 WHERE id=?", (debt_id,))
        self.conn.commit()
